(function () {
  ...
  const state = {
    ...
    expenseHistoryStart: "", expenseHistoryEnd: "", expenseHistoryPage: 1, expenseHistoryPageSize: 10,
  ...
  function renderAllocations() {
    ...
    if (state.allocationTab === "withdrawal") {
      const options = spendableAllocationRules().map(row => `<option value="${row.id}">${escapeHtml(row.name)} — saldo ${rupiah(allocationBalanceForRule(row))}</option>`).join("");
      content = `<section class="grid two allocation-withdraw-layout"><form id="allocationWithdrawalForm" class="card"><h4>Ambil Dana Pos</h4><div class="notice info">Pengambilan akan langsung mengurangi saldo pos yang dipilih dan masuk ke riwayat.</div><div class="form-grid"><div class="field full"><label>Pos pembagian</label><select id="allocationRule" required>${options}</select></div><div class="field"><label>Nominal</label><input id="allocationAmount" type="number" min="1" required></div><div class="field"><label>Tanggal</label><input type="date" value="${currentDate()}" readonly></div><div class="field full"><label>Catatan</label><input id="allocationNotes" required placeholder="Contoh: Keperluan Bapa"></div></div><button class="button primary section-gap" type="submit">Simpan pengambilan</button></form><article class="card"><h4>Saldo Pos Saat Ini</h4><div class="allocation-mini-list">${names.map(name => { const row = balances.find(item => item.name === name) || { balance: 0 }; return `<div><span>${escapeHtml(name)}</span><strong>${rupiah(row.balance)}</strong></div>`; }).join("") || '<div class="empty">Belum ada saldo.</div>'}</div></article></section>`;
    }
    ...
    return `<div class="page-head"><div><h3>Pos Pembagian</h3><p>Kelola saldo, pengambilan, riwayat, dan aturan setiap pos dalam satu halaman.</p></div></div>${tabs}${content}`;
  }

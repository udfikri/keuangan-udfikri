(function () {
  ...
  const state = {
    ...
    expenseHistoryStart: "", expenseHistoryEnd: "", expenseHistoryFunding: "all", expenseHistoryPage: 1, expenseHistoryPageSize: 10,
    allocationTab: "summary", allocationHistoryName: null, allocationHistoryPage: 1, allocationHistoryPageSize: 10, parsedImport: null
  };

  const titles = {
    dashboard: "Dashboard", sales: "Import & Tutup Buku", salary: "Gaji Karyawan",
    expenses: "Pengeluaran", allocations: "Pos Pembagian", reports: "Laporan & Kas", master: "Kelola Data", mySalary: "Gaji Saya"
  };

  const pageUrls = {
    dashboard: "dashboard.html", sales: "penjualan.html", salary: "gaji.html",
    expenses: "pengeluaran.html", allocations: "pos.html", reports: "laporan.html", master: "kelola-data.html",
  ...
  function renderPage() {
    const renderers = { dashboard: renderDashboard, sales: renderSales, salary: renderSalary, expenses: renderExpenses, allocations: renderAllocations, reports: renderReports, master: renderMaster, mySalary: renderMySalary };
  ...
  function renderExpenses() {
    ...
    const combinedExpenses = [
      ...state.expenses.map(row => ({ ...row, history_date: row.expense_date, funding_name: "Omzet Harian", funding_key: "daily", history_source: "expense" })),
      ...state.allocationWithdrawals.map(row => ({ ...row, history_date: row.withdrawal_date, funding_name: row.allocation_name, funding_key: `allocation:${row.allocation_name}`, history_source: "allocation", category: row.category || "Pengambilan Pos", description: row.description || row.notes || "-", expense_type: row.expense_type || "operational" }))
    ];
    const fundingFilterOptions = [...new Set(state.allocationWithdrawals.map(row => row.allocation_name).filter(Boolean))].sort((a, b) => a.localeCompare(b)).map(name => `<option value="allocation:${escapeHtml(name)}" ${state.expenseHistoryFunding === `allocation:${name}` ? "selected" : ""}>${escapeHtml(name)}</option>`).join("");
    const filteredExpenses = combinedExpenses.filter(row => {
      const date = String(row.history_date || "").slice(0, 10);
      const fundingMatches = state.expenseHistoryFunding === "all" || row.funding_key === state.expenseHistoryFunding;
      return fundingMatches && (!state.expenseHistoryStart || date >= state.expenseHistoryStart) && (!state.expenseHistoryEnd || date <= state.expenseHistoryEnd);
  ...
  function allocationLedgerRows(name) {
    const rows = [];
    state.reports.forEach(report => (Array.isArray(report.allocation_json) ? report.allocation_json : []).filter(item => ["fixed", "percent"].includes(item.type) && item.name === name).forEach(item => rows.push({ id: report.id, source: "earning", date: report.report_date, order: 1, description: item.type === "fixed" ? "Pembagian laba · nominal tetap" : `Pembagian laba · ${num(item.value)}%`, earning: num(item.amount), withdrawal: 0 })));
    state.allocationWithdrawals.filter(row => row.allocation_name === name).forEach(row => rows.push({ id: row.id, source: "withdrawal", date: row.withdrawal_date, order: 2, description: row.description || row.notes || "Pengambilan dana pos", earning: 0, withdrawal: num(row.amount) }));
    rows.sort((a, b) => String(a.date).localeCompare(String(b.date)) || a.order - b.order);
    let balance = 0;
    rows.forEach(row => { balance += row.earning - row.withdrawal; row.balance = balance; });
    return rows;
  }

  function renderAllocations() {
    if (!["summary", "history", "rules"].includes(state.allocationTab)) state.allocationTab = "summary";
    const balances = allocationBalances();
    const names = [...new Set([...spendableAllocationRules().map(row => row.name), ...balances.map(row => row.name)])].filter(name => { const rule = state.rules.find(row => row.name === name); return !rule || (rule.allocation_target !== "owner" && !/pemilik/i.test(name)); });
    if (!names.includes(state.allocationHistoryName)) state.allocationHistoryName = names[0] || null;
    const cards = names.map(name => balances.find(row => row.name === name) || { name, earned: 0, withdrawn: 0, balance: 0 }).map(row => `<article class="allocation-balance-card"><div class="allocation-card-head"><span>${escapeHtml(row.name)}</span><i class="fa-solid fa-wallet"></i></div><strong>${rupiah(row.balance)}</strong><div class="allocation-card-detail"><span>Terkumpul <b>${rupiah(row.earned)}</b></span><span>Terpakai <b>${rupiah(row.withdrawn)}</b></span></div></article>`).join("") || '<div class="empty allocation-empty">Belum ada pos pembagian.</div>';
    const tabs = `<div class="salary-tabs allocation-main-tabs"><button data-allocation-tab="summary" class="salary-tab ${state.allocationTab === "summary" ? "active" : ""}"><i class="fa-solid fa-chart-pie"></i> Ringkasan Pos</button><button data-allocation-tab="history" class="salary-tab ${state.allocationTab === "history" ? "active" : ""}"><i class="fa-solid fa-clock-rotate-left"></i> Riwayat</button><button data-allocation-tab="rules" class="salary-tab ${state.allocationTab === "rules" ? "active" : ""}" data-admin-action><i class="fa-solid fa-sliders"></i> Aturan Pos</button></div>`;
    let content = "";
    if (state.allocationTab === "summary") content = `<section class="card allocation-overview"><div class="section-title-row"><div><h4>Saldo Semua Pos</h4><p class="muted">Saldo berasal dari pembagian laba yang sudah ditutup, dikurangi seluruh pengambilan.</p></div></div><div class="allocation-balance-grid">${cards}</div></section>`;
    if (state.allocationTab === "history") {
      const selected = state.allocationHistoryName;
      const chronological = selected ? allocationLedgerRows(selected) : [];
      const newest = [...chronological].reverse();
      const pageCount = Math.max(1, Math.ceil(newest.length / state.allocationHistoryPageSize));
      state.allocationHistoryPage = Math.min(Math.max(1, state.allocationHistoryPage), pageCount);
      const start = (state.allocationHistoryPage - 1) * state.allocationHistoryPageSize;
      const visible = newest.slice(start, start + state.allocationHistoryPageSize);
      const totalEarned = chronological.reduce((total, row) => total + row.earning, 0), totalWithdrawn = chronological.reduce((total, row) => total + row.withdrawal, 0);
      const rows = visible.length ? visible.map(row => `<tr><td>${formatDate(row.date)}</td><td>${escapeHtml(row.description)}</td><td class="positive">${row.earning ? rupiah(row.earning) : "-"}</td><td class="negative">${row.withdrawal ? rupiah(row.withdrawal) : "-"}</td><td><strong>${rupiah(row.balance)}</strong></td><td>${row.source === "withdrawal" ? `<div class="button-row"><button class="button edit small" data-action="edit-allocation-expense" data-id="${row.id}">Edit</button><button class="button danger small" data-action="delete-allocation-withdrawal" data-id="${row.id}">Hapus</button></div>` : '<span class="pill">Tutup buku</span>'}</td></tr>`).join("") : '<tr><td colspan="6" class="empty">Belum ada transaksi pada pos ini.</td></tr>';
      const pages = Array.from({ length: pageCount }, (_, index) => index + 1).filter(page => page === 1 || page === pageCount || Math.abs(page - state.allocationHistoryPage) <= 1).map((page, index, list) => `${index && page - list[index - 1] > 1 ? '<span class="pagination-ellipsis">…</span>' : ""}<button class="ledger-page ${page === state.allocationHistoryPage ? "active" : ""}" data-allocation-history-page="${page}">${page}</button>`).join("");
      content = `<section class="card"><div class="section-title-row"><div><h4>Riwayat Saldo per Pos</h4><p class="muted">Transaksi terbaru tampil terlebih dahulu. Saldo dihitung dari transaksi paling lama.</p></div><label class="field ledger-page-size"><span>Baris</span><select id="allocationHistoryPageSize"><option value="10" ${state.allocationHistoryPageSize === 10 ? "selected" : ""}>10</option><option value="20" ${state.allocationHistoryPageSize === 20 ? "selected" : ""}>20</option><option value="50" ${state.allocationHistoryPageSize === 50 ? "selected" : ""}>50</option></select></label></div><div class="employee-ledger-tabs">${names.map(name => `<button class="employee-ledger-tab ${name === selected ? "active" : ""}" data-allocation-history-name="${escapeHtml(name)}">${escapeHtml(name)}</button>`).join("")}</div><div class="ledger-summary allocation-ledger-summary"><div><span>Nama pos</span><strong>${escapeHtml(selected || "-")}</strong></div><div><span>Total masuk</span><strong>${rupiah(totalEarned)}</strong></div><div><span>Total pengambilan</span><strong>${rupiah(totalWithdrawn)}</strong></div><div class="balance"><span>Saldo tersedia</span><strong>${rupiah(totalEarned - totalWithdrawn)}</strong></div></div><div class="table-wrap"><table><thead><tr><th>Tanggal</th><th>Keterangan</th><th>Penambahan</th><th>Pengambilan</th><th>Total Saldo</th><th>Aksi</th></tr></thead><tbody>${rows}</tbody></table></div><div class="ledger-pagination"><span>Menampilkan ${newest.length ? start + 1 : 0}–${Math.min(start + state.allocationHistoryPageSize, newest.length)} dari ${newest.length} transaksi</span><div><button class="ledger-page" data-allocation-history-page="${Math.max(1, state.allocationHistoryPage - 1)}" ${state.allocationHistoryPage === 1 ? "disabled" : ""}>Sebelumnya</button>${pages}<button class="ledger-page" data-allocation-history-page="${Math.min(pageCount, state.allocationHistoryPage + 1)}" ${state.allocationHistoryPage === pageCount ? "disabled" : ""}>Berikutnya</button></div></div></section>`;
    }
    if (state.allocationTab === "rules") {
      const ruleRows = state.rules.length ? state.rules.map(row => `<tr><td>${row.sort_order}</td><td><strong>${escapeHtml(row.name)}</strong></td><td>${row.rule_type === "fixed" ? "Nominal tetap" : "Persentase"}</td><td>${row.allocation_target === "owner" || (!row.allocation_target && /pemilik/i.test(row.name)) ? "Pemilik" : "Pos tabungan"}</td><td>${row.rule_type === "fixed" ? rupiah(row.value) : `${num(row.value)}%`}</td><td><span class="pill ${row.active ? "" : "off"}">${row.active ? "Aktif" : "Nonaktif"}</span></td><td><div class="button-row"><button class="button edit small" data-action="edit-rule" data-id="${row.id}">Edit</button><button class="button ${row.active ? "variant" : "success"} small" data-action="toggle-rule" data-id="${row.id}" data-active="${!row.active}">${row.active ? "Nonaktifkan" : "Aktifkan"}</button><button class="button danger small" data-action="delete-rule" data-id="${row.id}">Hapus</button></div></td></tr>`).join("") : '<tr><td colspan="7" class="empty">Belum ada aturan pos.</td></tr>';
      content = `<section class="grid two"><form id="ruleForm" class="card"><h4>Tambah Aturan Pos</h4><div class="form-grid"><div class="field full"><label>Nama pos</label><input id="ruleName" required placeholder="Contoh: Tabungan Bapa"></div><div class="field"><label>Jenis</label><select id="ruleType"><option value="fixed">Nominal tetap</option><option value="percent">Persentase</option></select></div><div class="field"><label>Tujuan</label><select id="ruleTarget"><option value="other">Pos tabungan</option><option value="owner">Pemilik</option></select></div><div class="field"><label>Nilai</label><input id="ruleValue" type="number" min="0" step="0.01" required></div><div class="field"><label>Urutan</label><input id="ruleOrder" type="number" min="1" value="99" required></div></div><button class="button primary section-gap" type="submit">Tambah pos</button></form><article class="card"><h4>Cara Pembagian</h4><div class="notice info">Pos nominal tetap dibagikan lebih dahulu. Setelah itu, sisa laba dibagi berdasarkan persentase aktif dan urutannya.</div></article></section><section class="card section-gap"><h4>Daftar Aturan Pos</h4><div class="table-wrap"><table><thead><tr><th>Urutan</th><th>Nama</th><th>Jenis</th><th>Tujuan</th><th>Nilai</th><th>Status</th><th>Aksi</th></tr></thead><tbody>${ruleRows}</tbody></table></div></section>`;
    }
    return `<div class="page-head"><div><h3>Pos Pembagian</h3><p>Pantau saldo dan riwayat setiap pos. Penggunaan dana dicatat satu kali melalui halaman Pengeluaran.</p></div><button class="button primary" data-go="expenses"><i class="fa-solid fa-receipt"></i> Catat pengeluaran</button></div>${tabs}${content}`;
  }
  ...
  function renderReports() {
    ...
    return `<div class="page-head"><div><h3>Laporan & Kas</h3><p>Rekap bulanan, pencocokan kas, dan backup.</p></div><label class="field"><span>Bulan laporan</span><input id="reportMonth" type="month" value="${month.month}"></label></div>
  ...
  function bindPageEvents() {
    ...
    if ($("#expenseHistoryFunding")) $("#expenseHistoryFunding").onchange = event => { state.expenseHistoryFunding = event.target.value; state.expenseHistoryPage = 1; renderPage(); };
    if ($("#resetExpenseHistory")) $("#resetExpenseHistory").onclick = () => { state.expenseHistoryStart = ""; state.expenseHistoryEnd = ""; state.expenseHistoryFunding = "all"; state.expenseHistoryPage = 1; renderPage(); };
    ...
    $$('[data-allocation-tab]').forEach(button => button.onclick = () => { state.allocationTab = button.dataset.allocationTab; renderPage(); });
    $$('[data-allocation-history-name]').forEach(button => button.onclick = () => { state.allocationHistoryName = button.dataset.allocationHistoryName; state.allocationHistoryPage = 1; renderPage(); });
    $$('[data-allocation-history-page]').forEach(button => button.onclick = () => { if (button.disabled) return; state.allocationHistoryPage = num(button.dataset.allocationHistoryPage) || 1; renderPage(); });
    if ($("#allocationHistoryPageSize")) $("#allocationHistoryPageSize").onchange = event => { state.allocationHistoryPageSize = num(event.target.value) || 10; state.allocationHistoryPage = 1; renderPage(); };
